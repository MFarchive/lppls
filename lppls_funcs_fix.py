...
class LPPLS(object):
    ...
    def plot_fit(self, show_tc=False):
        ...
        time_ord = [pd.Timestamp.fromordinal(d) for d in self.observations[0, :].astype('int64')]
    ...
    def compute_indicators(self, res, filter_conditions_config=None):
        ...
        for r in res:
            ...
            pos_conf = pos_qual_count / total_count if pos_count > 0 else 0 # MF modified code
            neg_conf = neg_qual_count / total_count if neg_count > 0 else 0 # MF modified code
        ...
        # does _fits have 2 results for the positive and negative best fit at each date?
    ...
    def plot_confidence_indicators(self, res):
        ...
        ax1_0.plot(ts, res_df['pos_conf'], label='bubble indicator (pos)', color='blue', alpha=0.5)
        ...
        ax2_0.plot(ts, res_df['neg_conf'], label='bubble indicator (neg)', color='red', alpha=0.5)
        ...
        # set labels
        ax1.set_ylabel('ln(price)')
        ax2.set_ylabel('ln(price)')

        ax1_0.set_ylabel('Positive bubble confidence')
        ax2_0.set_ylabel('Negative bubble confidence')
